...
class ClientesController extends Controller
{
    ...
    */
    ...
    public function store(Request $request){
        ...
        $request->validate([
            'correo' => 'required|min:4|unique:clientes',
        ...
        if ($correovalido==true && $passwordvalida==true) {
                if ($consultacorreo[0]['rol']=='admin') {
                    $telefono=$consultacorreo[0]['telefono'];
                    session(['tlf' => $telefono]);
                    session(['usu' => $request->get('correo')]);
                    return redirect()->route('menu3');
                }
                if ($consultacorreo[0]['rol']=='cliente') {
                    $telefono=$consultacorreo[0]['telefono'];
                    session(['tlf' => $telefono]);
                    session(['usu' => $request->get('correo')]);
